        ...
        <table class="view-stage-detail-table wiki-table">
            ...
            <tr>
                ...
                <td class="bonus" rowspan="2">
                    ...
                    <div>固定奖励：
                        <a href="<c:url value="/Wiki" />/${levelInfo.firstWinBonus.typeName}s/${levelInfo.firstWinBonus.name}" target="_self">
                ...
                <td class="value cards">
                    <c:forEach var="card" items="${levelInfo.deckInfo.cards}">
                        <div class="item">
                            <a href="<c:url value="/Wiki" />/Cards/${card.cardName}" target="_self">${card.cardName}</a>-${card.level}<c:if test="${card.extraSkill != null}">+<a href="<c:url value="/Wiki" />/Skills/${card.extraSkill.name}" target="_self">${card.extraSkill.name}</a></c:if>,
                        </div>
                    </c:forEach>
                    <div class="clear"></div>
                </td>
            ...
            <tr>
                <td class="value runes">
                    <c:forEach var="rune" items="${levelInfo.deckInfo.runes}">
                        <div class="item">
                            <a href="<c:url value="/Wiki" />/Runes/${rune.runeName}" target="_self">${rune.runeName}-${rune.level}</a>
                        </div>
                    </c:forEach>
                    <div class="clear"></div>
                </td>
