...
//        }
    ...
    public void resolvePreAttackSkills(CardInfo attacker, Player defender, int status) throws HeroDieSignal {
        for (SkillUseInfo skillUseInfo : attacker.getUsableNormalSkills()) {
            ...
            if (skillUseInfo.getType() == SkillType.透支 || skillUseInfo.getType() == SkillType.过载 || skillUseInfo.getType() == SkillType.勤学苦练) {
            ...
            } else if (skillUseInfo.getType() == SkillType.号角 || skillUseInfo.getType() == SkillType.集结旗帜 || skillUseInfo.getType() == SkillType.疾如风) {
                ...
                HandSword.apply(this, skillUseInfo, attacker, defender, 1);
            } else if (skillUseInfo.getType() == SkillType.火烧连营 || skillUseInfo.getType() == SkillType.彻骨霜火 || skillUseInfo.getType() == SkillType.妖狐火焰 || skillUseInfo.getType() == SkillType.侵略如火) {
                    ...
                    if(!(cardSkill != null && (cardSkill.getType() == SkillType.背水)))
                    {
                        for (CardInfo cardInfo : defenderPlayer.getField().getAliveCards()) {
                            for(SkillUseInfo skillUseInfo:cardInfo.getAllNormalSkills())
                            {
                                ...
                                {
                                    attackHero(cardInfo,attacker.getOwner(),skillUseInfo.getSkill(),damage*skillUseInfo.getSkill().getImpact()/100);
                                }
                            }
                        }
                    }
